<script>
    ...
    async function getRedditPosts(after = '') {
        let res = await axios.get(`https://www.reddit.com/r/TikTokCringe/top.json?t=year&limit=100${after !== '' ? `&after=t3_${after}` : ''}`);
        const start = DateTime.fromISO(startDate).toSeconds();
        const end = DateTime.fromISO(endDate).toSeconds();
        res = res.data.data.children.filter(post => {
    ...
    $: promise = getRedditPosts(after, startDate, endDate);
    $: lastPostId = promise.then(res => res[res.length - 1].data.id);

    const handleClick = () => {
        console.log('lastPostId', lastPostId);
        const temp = lastPostId.then(res => {
            console.log('res', res);
            after = res;
        });
    }
        ...
        {:then posts}
            ...
            <div class="grid grid-cols-3 grid-gap-2 rounded-md mb-4">
                <div class="col-span-2">
                    <video height="488px" preload="auto" class="max-h-[488px] !w-full" controls>
